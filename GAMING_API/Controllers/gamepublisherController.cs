...
namespace GAMING_API.Controllers
{
    ...
    public class gamepublisherController : ApiController
    {
        ...
        // GET api/<controller>
        public IEnumerable<Models.gamepublisher> Get()
        {
            ...
            var query = "SELECT * from game__publisher";
            _adapter = new SqlDataAdapter
            {
                SelectCommand = new SqlCommand(query, _conn)
            };
            _adapter.Fill(_dt);
            List<Models.gamepublisher> gameb = new List<Models.gamepublisher>(_dt.Rows.Count);
            if (_dt.Rows.Count > 0)
            {
                foreach (DataRow dev in _dt.Rows)
                {
                    gameb.Add(new Readgamepublisher(dev));
                }
            }
            return gameb;
        }
